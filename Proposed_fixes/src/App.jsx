...
function App() {
  ...
  // 2. Update URL when year changes
  useEffect(() => {
    ...
    window.history.pushState(null, '', newUrl); // Adds to history
  }, [year]);
  ...
  // 5. Fetch contributions when year changes
  useEffect(() => {
    const from = `${year}-01-01T00:00:00Z`;
    const to = `${year}-12-31T23:59:59Z`;
          ...
          {years.map(yr => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-sm transition ${
                yr === year ? 'bg-green-500 text-black' : 'bg-gray-700 hover:bg-gray-600'
              }`}
            >
              {yr}
            </button>
